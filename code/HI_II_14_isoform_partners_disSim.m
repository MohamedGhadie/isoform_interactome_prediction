...
diffGeneDisSim_file = [processed_data_dir '/HI-II-14_diffGene_disSim.mat'];

if (load_processed_data == 1) && (exist(diffGeneDisSim_file, 'file') == 2)
    disp('Loading disease similarity results for pairs of proteins -');
    disp(['interacting with products of different genes from file ' diffGeneDisSim_file]);
    load(diffGeneDisSim_file)
else
    disp('Calculating disease similarity results for pairs of proteins -');
    ...
    for i = 1:numGenes-1
        if ~mod(i,1000)
            disp(['- Calculated disease similarity for ' num2str(i) ' genes out of ' num2str(numGenes) ' genes']);
        end
        if geneNumDis(i) > 0
            p1_disProfile = geneDisI(i,:);
            for j = i+1:numGenes
                if (geneNumDis(j) > 0) && numCommonPartners(i,j) == 0
                    c = c + 1;
                    p2_disProfile = geneDisI(j,:);
    ...
    if save_processed_data == 1
        ...
        disp('Disease similarity results for pairs of proteins -');
